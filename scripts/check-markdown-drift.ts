#!/usr/bin/env tsx
/**
 * Guard against silent drift between the two copies of MarkdownFormatter.
 *
 ...
 * *set of exports and component-level definitions* — without being noisy
...
const APP_FILE = resolve(
  REPO_ROOT,
  "app/src/components/blog/MarkdownFormatter.tsx",
);
const WEBSITE_FILE = resolve(
  REPO_ROOT,
  "website/src/components/blog/MarkdownFormatter.tsx",
);

// Names that are allowed to exist in only one copy. Add entries here when
// a genuine, intentional divergence is introduced; require a PR comment
// explaining why the two files differ on this identifier.
const ALLOW_ONLY_IN_APP = new Set<string>([
  // `useRobotoMonoFont` was introduced in the app/ copy only (see #980).
...
const ALLOW_ONLY_IN_WEBSITE = new Set<string>([
  // `Th` header cell component is only defined as a standalone export in
  // the website/ copy (app/ inlines it).
  "Th",
]);
...
function main(): number {
  ...
  const webSrc = readFileSync(WEBSITE_FILE, "utf8");

  const appDecls = extractTopLevelDecls(appSrc);
  const webDecls = extractTopLevelDecls(webSrc);

  const onlyInApp = [...appDecls]
    .filter((d) => !webDecls.has(d) && !ALLOW_ONLY_IN_APP.has(d))
    .sort();
  const onlyInWeb = [...webDecls]
    .filter((d) => !appDecls.has(d) && !ALLOW_ONLY_IN_WEBSITE.has(d))
    .sort();

  if (onlyInApp.length === 0 && onlyInWeb.length === 0) {
    console.log(
      "MarkdownFormatter drift check: PASS (top-level API in sync).",
    );
    return 0;
  }

  console.error("MarkdownFormatter drift check: FAIL");
  console.error("");
  console.error(
    "The two MarkdownFormatter copies have diverged in their top-level",
  );
  console.error(
    "API surface. Either port the change to the other file, or add the",
  );
  console.error(
    "identifier to ALLOW_ONLY_IN_APP / ALLOW_ONLY_IN_WEBSITE in",
  );
