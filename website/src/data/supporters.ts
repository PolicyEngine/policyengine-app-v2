import { Supporter } from "@/components/static/SupporterCard";
import { SupportedProjectData } from "@/components/static/SupportedProject";
...
export const supporters: Supporter[] = [
  ...
  },
  {
    ...
    description:
      "a nonprofit dedicated to ending the cycle of family poverty and homelessness.",
  },
  {
    id: "dylan",
...
export const projects: SupportedProjectData[] = [
  {
    title:
...
export function getSupportersWithTotals() {
  const result = supporters.map((supporter) => {
    const supporterProjects = projects
      .filter((p) => p.supporterId === supporter.id)
      .sort((a, b) => b.awardDate.localeCompare(a.awardDate));

    const total = supporterProjects.reduce(
      (sum, project) => sum + toUSD(project.amount, project.currency),
      0,
    );
