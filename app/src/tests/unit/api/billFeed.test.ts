...
describe('provisionsFromReformParams', () => {
  test('given dated value maps then every scheduled value is preserved', () => {
    const provisions = provisionsFromReformParams({
      'gov.irs.credits.ctc.amount.base[0].amount': {
        '2026-01-01.2026-12-31': 2500,
        '2027-01-01.2100-12-31': 3000,
      },
    ...
    expect(provisions).toEqual([
      ...
      },
      {
        ...
        values: [{ startDate: '2026-01-01', endDate: '2100-12-31', value: 300 }],
      },
    ]);
  });
  ...
  test('given null or malformed params then no provisions are invented', () => {
    ...
    expect(provisionsFromReformParams({ 'gov.test.invalid': { '2026-01-01': 1 } })).toEqual([]);
  });
});
